...
UI/Components/
  ...
  Filters/         扫描器可点选项（`TappableValue`；方向、周期、窗口、最低价、最低量、RSI、ADX、DI、类型、最低市值；标题在左选项同一行，挤不下横向滚动；单独成列时过宽换行；点击高度 ≥ 44pt。ATR / 阶梯窗口请求仍传 `barCount`；盘前窗口请求传 `period`）
  Stats/           迷你图（订阅监视与持仓列表折线、扫描器结果蜡烛 + VWAP）；仪表盘统计图以后也放这里
```
