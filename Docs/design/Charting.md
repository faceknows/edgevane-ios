...
交易 Tab 同时展示多只订阅代码时，**不要**每行嵌两套 `ChartSurface`。扫描器结果同样不嵌 `ChartSurface`。迷你图走 `SparklineView`（Swift Charts 折线；蜡烛与 iOS 15 用 Canvas 兜底。无成交量、无面积填充、无十字光标、不缩放；单点用点而不是空白折线，X 轴范围不能退化成 0...0）。迷你图只吃 `SparklinePlot`：`.line(values:overlay:)` 或 `.candles(bars:vwap:)`，不要平行塞 style / values / bars。叠加须与主序列条数对齐，对不齐就不画叠加。交易 Tab 仍把分钟/秒聚合成收盘折线。**扫描器分钟图只画 1 分钟蜡烛 + 蓝色 VWAP**（VWAP 用 1 分钟棒算出后对齐到各根蜡烛；不要再画收盘折线）；交易 Tab 监视迷你图仍不画 VWAP。迷你图空数据由 `SparklinePane` 可配置：扫描器成功拉到空分钟棒时展示空态文案，不要留整块可点空白；交易 Tab 不传文案，空数据留空、不准假平线。秒图只取相对现在约 10 分钟内、且不晚于现在的棒；未来棒不进环缓。无新行情时按下一次出窗调度，插入新棒要唤醒调度器，不每秒刷新整表；无棒时长时间休眠，过期循环重叠时插入仍能打断当前休眠。分钟拉取失败要有失败提示和重试，不能只打日志；有上次折线时也要提示，不能把旧图当成最新。卡片进详情是按钮；重试是独立按钮，不触发进详情。离开本页要取消未完成的分钟拉取（含重试）。
...
| 扫描器结果迷你图 | 当日 regular 1 分钟蜡烛；未订阅无秒图；成功空数组走空态文案，不要留整块可点空白 | VWAP（蓝色；1 分钟算完再对齐到各根蜡烛） | 无 | — | false（不走 `ChartSurface`） |
