...
PriceLine
  id            如 "prevClose"、"open"
  price
  title         已本地化或由 UI 传入 L10n key 的结果；图上不把该字画进 K 线
  dashed        Bool
...
ChartModel
  ...
  seriesID      标的 + 会话 + 日期。换代码或换日要铺满并清空十字选中；同一序列的第一根 OHLC 更新不要铺满、也不要丢掉当前选中棒
```
...
```text
ChartEvent
  picked(Bar)           十字光标选中某根；换 seriesID 后清空，即使时间戳与上一标重叠
...
左右平移和双指缩放由库内部消化，不必每帧回传给页面业务。详情分钟主图与纳指对照共用同一段**墙钟时间**起止：切 1/3/5 时保持该窗口，不要按逻辑索引对齐（1 分钟 300 根和 5 分钟 60 根不是同一时段）。分钟主图是源，平移/缩放/双击铺满后纳指跟到同一起止；纳指作为从属图关闭平移、缩放和双击复位，时间窗只跟主图。纳指自己的数据更短时仍用这段时间，不要反过来把主图裁短。盘前、盘后、日线、秒图不同步。双击图区复位：已加载的全部棒铺满当前图区（与第一次出图相同）；分钟主图双击后把新窗口同步给纳指。新棒增量更新不要重新铺满：放大看中间时窗口不变；当前已看见最后一根时新棒跟进。单指上下滑**不**给图表（详情叠了多张图，要滚整页）；左右滑仍平移 K 线（纳指对照除外）。可交互源图左滑时最新一根钉在可见区右缘，不要拖出未来空白；放大后看历史时最新一根可以出画，再左滑看回来。恢复同一墙钟窗口时短序列可以露出窗口内的空白；源图关掉库右锁、自行把逻辑 `to` 钳在该窗口右界，不能再往未来拖。标记必须跟着缩放平移，不能用 SwiftUI 盖一层对不齐的点。

## 4. 谁组装 ChartModel

`Market`（或详情 Store）组装，**不在 View 里算 VWAP**。

| 图 | bars 来源 | overlays | priceLines | showVolume |
| --- | --- | --- | --- | --- |
| 盘中分钟 1/3/5 | 1 分钟仓库，3/5 现场聚合 | VWAP（开关开；按当前 1/3/5 周期的棒算） | 昨收、今开（有快照才有） | 股票 true；正在看 COMP / NASDAQ 则 false |
| 盘前 / 盘后 | 对应会话棒，展示按 5 分钟聚合 | 无（第一期） | 可选 | true |
| 日线 | 日线仓库 | 无 | 无 | 股票 true；COMP / NASDAQ false |
| 秒 1/5/10/30（详情缺省 1 秒折线） | 秒线环缓聚合 | 无 | 无 | true |
| 纳指对照 | **第二套** `ChartSurface`（矮图），只画 COMP。不要 VXX，也不要把纳指叠进主图价格轴。**1/3/5 与蜡烛/线跟盘中分钟同一套 `ChartChrome`**；**可见时间起止与盘中分钟相同**，切周期保持该窗口。没有自己的周期开关，也不画 VWAP | — | — | false |
| 历史分钟 / 复盘 | 指定日期拉取，不进「今天」仓库 | VWAP | 可选昨收；**markers = 当天该标的成交** | 与盘中分钟相同 |
...
复盘没有实时秒线，视口规则与其它主图相同：放大看中间不拽回去。
...
- `PriceLine` → `createPriceLine` 只画虚线（`title` 留空、关掉 axisLabel，避免图内色块和右轴价盒）。昨收 / 今开价格画在右侧价格轴（图区外面）对应价位，与 Y 轴刻度同一列，颜色与虚线一致、无底；彼此或贴现价过近时纵向错开。`OverlayLine` → `LineSeries`；`ChartMarker` → 成交价上的蓝色 ∨（买）与 ∧（卖），图上不画数量文字。
...
- 手势：单指上下滑交给外层页面滚动（详情叠了多张图）；左右平移、双指缩放仍由图表消化。可交互源图锁右缘：左滑不能把最新一根推出右缘；放大后看历史时最新一根可以出画。纳指对照不锁右缘，短序列的逻辑 `to` 可以超过最后一根。切 1/3/5 恢复超出最后一根的窗口时，源图不要开库的 `fixRightEdge`（v4 会把 `rightOffset` 钳成 0），改为监听逻辑范围钳在该窗口右界；`visibleTimeRange` 在逻辑钳制之后按逻辑索引反推墙钟发布，不要用库会裁掉未来空白的 visible-time 回调。双击图区调用与第一次出图相同的铺满（时间轴 `fitContent`，价格轴恢复自动缩放）；缩放或平移后都能这样复位。增量新棒不要再 `fitContent`：已放大看中间时保持逻辑/墙钟窗口；当前已看见最后一根时把窗口右移让新棒进来。详情纳指对照关闭这些时间轴交互，只跟分钟主图的可见时间。
...
2. 盘前 / 盘后（有数据才出现）
3. 日线（偏好默认关）chrome + surface
4. **纳指对照矮图**（偏好默认开，无 VXX）surface，**没有自己的 chrome**
5. `ChartChrome`：1M/3M/5M、蜡烛/线、VWAP（只作用分钟主图）+ 盘中 `ChartSurface`（可叠今日成交 `markers`）。切周期/形态时纳指矮图一起变，**可见时间起止跟分钟主图相同且切 1/3/5 保持不变**（换周期后用当前 1/3/5 的棒时长重算柱宽，不要从首两根去猜、也不要沿用上一周期的逻辑索引把柱压扁），VWAP 不画在纳指上
6. RSI/ADX/+DI/-DI/ATR/AtcPct 数字徽标（随当前分钟周期重算，不是副图、不用 summaries 上的指标）
7. 秒图 chrome + surface（有秒线才出现）
8. 价格滑条（仅有秒图时；区间来自秒图高低，不要用最新价兜底出滑条）。滑条上方居左显示当前选定限价，不要居中（拖动时会被手指挡住），也不要「拖动选择限价」之类提示
9. 成交价条 + **Q 盘口**（买/卖价与数量；`quote` 为主，快照兜底，没有盘口才显示 —）。放在盘中分钟图下方、买卖按钮上方，不要放在正文顶部
10. 交易条（有仓则 Type / Qty / Filled 与相对中间价%；Liquidate @ Limit / Take Profit / Stop / Buy @ Bid / Sell @ Ask）；可退订

未订阅：无 9 的盘口、无 7–8 与 10 的秒图/滑条/交易条；盘中分钟图下仍显示摘要最新价；底部「加入实时订阅」。
...
- 无 VXX。纳指是独立矮图。
- 第一次出图铺满已加载棒；缩放或平移后双击同样铺满。新棒不要把当前窗口重新铺满。切 1/3/5 时分钟主图与纳指对照保持同一可见时间起止，不要整段跳窗。操作纳指图不会改时间窗。
- 详情图最高/最低在实际价位旁用 `--价` / `价--`；当前价先画虚线再画数字盖住虚线，不横贯整图、不穿过数字。
- 扫描器迷你图有美东时间轴、右侧价格轴、最高/最低数字、当前价虚线（只从最后一根收到右轴）；交易 Tab / 持仓并排迷你图有时间轴与高低，不画右侧价格轴；仍不嵌 `ChartSurface`。
