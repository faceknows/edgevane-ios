...
| 订阅监视迷你图 | 当日 regular 1 分钟 + 秒线环缓，本页聚合成收盘序列 | 无 | 无（现价虚线由 `SparklineView` 画，不是 priceLines） | — | false（不走 `ChartSurface`） |
...
- VWAP：`Charting/Indicators/VWAP.swift`，典型价 `(h+l+c)/3`，按量累加。详情 / 复盘按 **当前周期** 的棒算（1M 用 1 分钟棒，3M/5M 用聚合后的棒），不要把 1 分钟 VWAP 叠到 3/5 分钟图上。扫描器只有 1 分钟蜡烛，仍用 1 分钟棒算。
