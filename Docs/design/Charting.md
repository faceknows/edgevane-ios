...
左右平移和双指缩放由库内部消化，不必每帧回传给页面业务。详情分钟主图与纳指对照共用同一段**墙钟时间**起止：切 1/3/5 时保持该窗口，不要按逻辑索引对齐（1 分钟 300 根和 5 分钟 60 根不是同一时段）。分钟主图是源，平移/缩放/双击铺满后纳指跟到同一起止；纳指作为从属图关闭平移、缩放和双击复位，时间窗只跟主图。纳指自己的数据更短时仍用这段时间，不要反过来把主图裁短。盘前、盘后、日线、秒图不同步。双击图区复位：已加载的全部棒铺满当前图区（与第一次出图相同）；分钟主图双击后把新窗口同步给纳指。单指上下滑**不**给图表（详情叠了多张图，要滚整页）；左右滑仍平移 K 线（纳指对照除外）。可交互源图左滑时最新一根钉在可见区右缘，不要拖出未来空白；放大后看历史时最新一根可以出画，再左滑看回来。恢复同一墙钟窗口时短序列可以露出窗口内的空白；源图关掉库右锁、自行把逻辑 `to` 钳在该窗口右界，不能再往未来拖。标记必须跟着缩放平移，不能用 SwiftUI 盖一层对不齐的点。
