...
| 屏 | 行为 |
| --- | --- |
| 持仓列表 | 刷新；顶部 **浮动盈亏合计** 与 **今日盈亏** 各自带可见标签。每行：方向、数量、成本、市值、浮动盈亏（均有可见标签；成本用 `costBasis`，与交易条一致）、分钟/秒迷你折线（与交易 Tab 同一套 `WatchSparklinePair`，调用方组装 `SparklinePlot`）、最新价与 bid/ask。点卡片进标的详情。秒图与盘口仅已订阅才有实时数据。列表 **不下单**；加仓与平仓都走已订阅标的详情交易条 |
| 组合 | 净值、购买力、现金、今日/总盈亏 |
