...
交易弹窗必须常驻显示代码、方向、盘口（最新成交、bid/ask 与数量）和模拟或实盘。数量提供 1/3、1/2、1、2、3 倍快捷选择，并与价格一样支持直接输入和减/加微调；点「确认」通过校验后立即执行，不再弹出二次确认框。不要在确认按钮上方再写一整行代码/方向/价格/股数摘要（标题和表单已有）。确认按钮旁须写明模拟或实盘（滑条的买入/卖出同样），避免滚出标题后误下实盘单。
...
| 屏 | 行为 |
| --- | --- |
| 持仓列表 | 刷新；顶部 **浮动盈亏合计** 与 **今日盈亏** 各自带可见标签。每行：方向、数量、成本、市值、浮动盈亏（均有可见标签；成本用 `costBasis`，与详情一致）、分钟/秒迷你折线（与交易 Tab 同一套 `WatchSparklinePair`，调用方组装 `SparklinePlot`）、最新价与 bid/ask。点标题进持仓详情，点图进标的详情。秒图与盘口仅已订阅才有实时数据。列表 **不下单、不加仓**；加仓走已订阅标的详情交易条，平仓走持仓详情 |
