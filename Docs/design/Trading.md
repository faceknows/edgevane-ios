...
交易弹窗必须常驻显示代码、方向、盘口（最新成交、bid/ask 与数量）和模拟或实盘。数量提供 1/3、1/2、1、2、3 倍快捷选择，并与价格一样支持直接输入和减/加微调；点「确认」通过校验后立即执行，不再弹出二次确认框。不要在确认按钮上方再写一整行代码/方向/价格/股数摘要（标题和表单已有）。确认按钮旁须写明模拟或实盘（滑条的买入/卖出同样），避免滚出标题后误下实盘单。凭证失效、无当前账户或 `trading_blocked` 时确认禁用并显示原因（弹窗本身不自动关掉）。
