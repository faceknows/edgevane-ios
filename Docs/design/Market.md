...
目录屏列出第一期 8 项（需求 01）。  
结果屏 **一个** `ScreenerResultsView`：标题、筛选（仅价格斜率等有参数的扫描器注入）、`SymbolRow` + 当日盘中 1 分钟迷你蜡烛（叠加蓝色 VWAP）、点进详情。分钟棒与交易 Tab、详情共用 `BarStore`、`SubscriptionWatchSession`、`SparklineView`；列表里重复的代码只拉一次。**扫描结果默认未订阅，不画秒图、不嵌 `ChartSurface`。** VWAP 用 1 分钟棒在 `Indicators` 算完再对齐到各根蜡烛；不要再画收盘折线。分钟棒**成功返回空数组**时展示空态文案（`SparklinePane` 可配置），不要留整块空白可点图区；尚未拉完或失败走加载/重试。交易 Tab 不传空态文案，空数据仍留空。
...
- 八个扫描器都能进列表并打开详情；结果行能看到当日 1 分钟蜡烛与蓝色 VWAP，没有秒图。无当日分钟棒时图区空态，不要留大块可点空白。
- 未订阅无秒图、无交易条；订阅后出现，并能退订。
- 切 1/3/5 分钟与蜡烛/线、VWAP 符合图表设计。无 VXX。
- 历史分钟 / 复盘不改变详情里「今天」的棒；有成交时图上能看到买卖点。
- 交易 Tab 能加/删订阅且不出现升级页；断线时能看出未连接。
- 已订阅列表能同时看到多只代码的当日分钟折线与秒级折线；无成交量柱。点卡片进详情。分钟刷新失败时即使还有旧折线也要显示失败和重试；点重试不进详情。
