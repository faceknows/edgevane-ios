...
- `SubscriptionWatchSession` 进入本页后为 `me` 里每个代码拉/轮询当日分钟棒（与详情共用 `BarStore`，有界并发）。摘要预取与分钟监视并行，分钟图不等摘要回来。离开本页取消未完成请求（含用户点的重试；含 `BarStore` 底层 HTTP；最后一名等待者离开则不写入共享缓存），不把结果写回本页。秒线仍只靠已订阅的 Socket，本页不另拉。无秒棒时过期调度长时间休眠，新棒插入再唤醒，不准每秒空转。
